// ============================================================
//  StdWork v3.1 - Core Logic (external file)
//  Loaded by StdWorkv3.1.html
// ============================================================

        const STDWORK_VERSION = '3.1';
        const WACT_CONTRIB_PREF_KEY = 'stdwork_wact_contrib_v21';
...
// ==================== 1. GLOBAL STATE ====================
        ...
        let showWactContribution = localStorage.getItem(WACT_CONTRIB_PREF_KEY) !== '0';
        ...
        const VA_CATEGORIES = ['value-added', 'necessary-nva', 'waste'];

        function normalizeVaCategory(va) {
            return VA_CATEGORIES.includes(va) ? va : 'value-added';
        }

        function getVaCategoryLabel(va) {
            const labels = { 'value-added': 'VA', 'necessary-nva': 'NNVA', 'waste': 'Waste' };
            return labels[normalizeVaCategory(va)] || 'VA';
        }

        function getVaCategoryFullLabel(va) {
            const labels = {
                'value-added': 'Value-Added',
                'necessary-nva': 'Necessary NVA',
                'waste': 'Waste'
            };
            return labels[normalizeVaCategory(va)] || 'Value-Added';
        }

        function normalizeLapFields(lap) {
            if (!lap) return lap;
            lap.vaCategory = normalizeVaCategory(lap.vaCategory);
            if (lap.keyPoints == null) lap.keyPoints = '';
            if (lap.reasonsWhy == null) lap.reasonsWhy = '';
            return lap;
        }

        function defaultLapStdWorkFields() {
            return { keyPoints: '', reasonsWhy: '', vaCategory: 'value-added' };
        }

        function computeStudyVaBreakdown(studyOrLaps, allowance) {
            const lapsArr = Array.isArray(studyOrLaps) ? studyOrLaps : (studyOrLaps?.laps || []);
            let vaMs = 0;
            let nnvaMs = 0;
            let wasteMs = 0;
            let totalMs = 0;
            lapsArr.filter(isRegularElement).forEach(lap => {
                const st = getLapStats(lap, allowance);
                if (!st.standardTime) return;
                totalMs += st.standardTime;
                const cat = normalizeVaCategory(lap.vaCategory);
                if (cat === 'necessary-nva') nnvaMs += st.standardTime;
                else if (cat === 'waste') wasteMs += st.standardTime;
                else vaMs += st.standardTime;
            });
            return {
                vaMs,
                nnvaMs,
                wasteMs,
                totalMs,
                pctVa: totalMs > 0 ? Math.round((vaMs / totalMs) * 100) : null
            };
        }
        ...
        function syncWactContributionToggleUI() {
            ...
            btn.classList.toggle('border-amber-400', showWactContribution);
            btn.classList.toggle('shadow-md', showWactContribution);
            btn.classList.toggle('shadow-amber-500/25', showWactContribution);
            btn.classList.toggle('bg-zinc-800', !showWactContribution);
            btn.classList.toggle('text-zinc-300', !showWactContribution);
            btn.classList.toggle('border-zinc-700', !showWactContribution);
        ...
        function toggleWactContribution() {
            ...
            localStorage.setItem(WACT_CONTRIB_PREF_KEY, showWactContribution ? '1' : '0');
            syncWactContributionToggleUI();
            renderLaps();
        }

        function showToast(message, type = 'success') {
            let el = document.getElementById('stdwork-toast');
            if (!el) {
            ...
            el.className = [
                'fixed bottom-6 left-1/2 -translate-x-1/2 z-[200] px-5 py-3 rounded-2xl text-sm font-semibold shadow-xl',
                'transition-all duration-300 max-w-[90vw] text-center',
                type === 'success' ? 'bg-emerald-600 text-white' : 'bg-amber-500 text-zinc-950'
            ].join(' ');
        ...
        function renderLaps() {
            ...
            laps.forEach((lap, index) => {
                ...
                const vaCat = normalizeVaCategory(lap.vaCategory);
                const vaColorClass = vaCat === 'waste' ? 'text-red-400 border-red-800/60 bg-red-950/30'
                    : (vaCat === 'necessary-nva' ? 'text-amber-400 border-amber-800/60 bg-amber-950/30' : 'text-emerald-400 border-emerald-800/60 bg-emerald-950/30');
                let vaHTML = '';
                if (isRegularElement(lap)) {
                    if (readOnly || isCycleMode) {
                        vaHTML = `<span class="inline-flex text-[10px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded border ${vaColorClass}">${getVaCategoryLabel(vaCat)}</span>`;
                    } else {
                        vaHTML = `<select onchange="setLapVaCategory(${index}, this.value)" class="va-category-select bg-zinc-950 border border-zinc-700 rounded-lg px-1.5 py-1 text-[10px] min-h-[32px] max-w-[6.5rem] ${vaColorClass}" title="Value-added classification for waste analysis">
                            <option value="value-added" ${vaCat === 'value-added' ? 'selected' : ''}>VA</option>
                            <option value="necessary-nva" ${vaCat === 'necessary-nva' ? 'selected' : ''}>NNVA</option>
                            <option value="waste" ${vaCat === 'waste' ? 'selected' : ''}>Waste</option>
                        </select>`;
                    }
                } else {
                    vaHTML = `<span class="text-[10px] text-zinc-600">—</span>`;
                }

                const hasStdWorkNotes = !!(lap.keyPoints?.trim() || lap.reasonsWhy?.trim());
                const stdWorkBtn = isRegularElement(lap)
                    ? `<button type="button" onclick="toggleLapStdWorkRow(${index})" class="text-xs px-2 py-1.5 rounded-xl border min-h-[36px] min-w-[36px] ${hasStdWorkNotes ? 'border-amber-500/50 text-amber-400 bg-amber-950/20' : 'border-zinc-700 text-zinc-400 bg-zinc-800 hover:bg-zinc-700'}" title="Key Points & Reasons Why" aria-label="Key Points and Reasons Why"><i class="fa-solid fa-list-check"></i></button>`
                    : '';
                ...
                tr.innerHTML = `
                    ...
                    <td class="hidden sm:table-cell py-2 px-2 sm:py-3 sm:px-5">${vaHTML}</td>
                    ...
                    <td class="py-2 px-2 sm:py-3 sm:px-5 text-right w-14 sm:w-20">
                        ${stdWorkBtn}
                        <button onclick="openElementalModal(${index})" class="text-xs px-2 sm:px-3 py-1.5 bg-zinc-800 hover:bg-amber-500 hover:text-zinc-950 rounded-xl mr-1 min-h-[36px] min-w-[36px]" title="${readOnly ? 'View element details' : 'Time observations for this element'}" aria-label="${readOnly ? 'View element details' : 'Time observations'}"><i class="fa-solid fa-${readOnly ? 'eye' : 'clock'}"></i></button>
                        ${deleteButton}
                    </td>
                `;
                ...
                if (isRegularElement(lap)) {
                    const stdWorkRow = document.createElement('tr');
                    stdWorkRow.id = `lap-stdwork-row-${index}`;
                    stdWorkRow.className = 'hidden bg-zinc-950/60';
                    const kpVal = escapeStdDocHtml(lap.keyPoints || '');
                    const rwVal = escapeStdDocHtml(lap.reasonsWhy || '');
                    const fieldDisabled = readOnly ? 'disabled' : '';
                    const voiceBtn = readOnly ? '' : (field) =>
                        `<button type="button" onclick="startVoiceToField('lap-${field}-${index}')" class="min-h-[44px] min-w-[44px] px-3 rounded-xl bg-zinc-800 hover:bg-amber-500/20 border border-zinc-700 text-zinc-400" title="Voice input" aria-label="Voice input for ${field}"><i class="fa-solid fa-microphone"></i></button>`;
                    stdWorkRow.innerHTML = `
                        <td colspan="9" class="py-3 px-3 sm:px-5">
                            <div class="text-[10px] font-semibold tracking-widest text-amber-400 mb-2">KEY POINTS &amp; REASONS WHY</div>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    <label class="text-[10px] text-zinc-500 block mb-1" for="lap-keyPoints-${index}">Key Points (method / safety / quality)</label>
                                    <div class="flex gap-2">
                                        <textarea id="lap-keyPoints-${index}" rows="2" ${fieldDisabled} onchange="updateLapStdWorkField(${index}, 'keyPoints', this.value)" class="flex-1 bg-zinc-900 border border-zinc-700 rounded-xl px-3 py-2 text-sm min-h-[52px] resize-y" placeholder="Grip here, verify torque, PPE…">${kpVal}</textarea>
                                        ${readOnly ? '' : voiceBtn('keyPoints')}
                                    </div>
                                </div>
                                <div>
                                    <label class="text-[10px] text-zinc-500 block mb-1" for="lap-reasonsWhy-${index}">Reasons Why (why this exact method)</label>
                                    <div class="flex gap-2">
                                        <textarea id="lap-reasonsWhy-${index}" rows="2" ${fieldDisabled} onchange="updateLapStdWorkField(${index}, 'reasonsWhy', this.value)" class="flex-1 bg-zinc-900 border border-zinc-700 rounded-xl px-3 py-2 text-sm min-h-[52px] resize-y" placeholder="Prevents misassembly, reduces rework…">${rwVal}</textarea>
                                        ${readOnly ? '' : voiceBtn('reasonsWhy')}
                                    </div>
                                </div>
                            </div>
                            <div class="sm:hidden mt-2">${vaHTML}</div>
                        </td>`;
                    tbody.appendChild(stdWorkRow);
                }
        ...
        function finalizeAndSaveStudy() {
            ...
            const vaBreakdown = computeStudyVaBreakdown(elementsWithStandards, allowance);
            ...
            const studyPayload = {
                ...
                ownerCell: area,
                metrics: {
                    wactSec, taktSec, headroomPct, taktDemand: demand, taktShiftHours: shiftHrs, taktBreaksMin: breaksMin,
                    pctVa: vaBreakdown.pctVa,
                    vaMs: vaBreakdown.vaMs,
                    nnvaMs: vaBreakdown.nnvaMs,
                    wasteMs: vaBreakdown.wasteMs
                }
            };
        ...
        function renderLineReportContent() {
            ...
            table.innerHTML = `
                <thead class="text-zinc-400 border-b border-zinc-800">
                    <tr>
                        ...
                        <th class="text-right py-2 px-3">%VA</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-zinc-800"></tbody>
            `;
            ...
            stationKeys.forEach(stationKey => {
                ...
                headerTr.innerHTML = `
                    <td colspan="6" class="py-2 px-3 font-semibold text-emerald-400">
                        ${stationKey}
                ...
                groupStudies.forEach(study => {
                    ...
                    const va = study.metrics?.pctVa != null
                        ? study.metrics
                        : computeStudyVaBreakdown(study, study.allowancePct || 12);
                    const pctVaDisplay = va.pctVa != null ? `${va.pctVa}%` : '—';
                    const tr = document.createElement('tr');
                    tr.innerHTML = `
                        ...
                        <td class="py-2 px-3 text-right font-mono">${pctVaDisplay}</td>
                    `;
                    tbody.appendChild(tr);
                });
            });
        ...
        function exportLineReportCSV() {
            ...
            csv += `\n`;
            csv += 'Area,Station,Study Name,Elements,WACT (sec),Takt (sec),Headroom (%),Pct VA,WACT VA (ms),WACT NNVA (ms),WACT Waste (ms),Allowance (%),Date,Created By,Workflow\n';
            const stationGroups = groupStudiesByStation(areaStudies);
        ...
        function toggleLapStdWorkRow(index) {
            const row = document.getElementById(`lap-stdwork-row-${index}`);
            if (row) row.classList.toggle('hidden');
        }

        function updateLapStdWorkField(index, field, value) {
            if (isReadOnlyMode() || !laps[index]) return;
            if (field !== 'keyPoints' && field !== 'reasonsWhy') return;
            laps[index][field] = value;
            saveToStorage();
        }

        function setLapVaCategory(index, value) {
            if (isReadOnlyMode() || !laps[index]) return;
            laps[index].vaCategory = normalizeVaCategory(value);
            renderLaps();
            saveToStorage();
        }

        function startVoiceToField(inputId) {
            ...
            rec.onerror = () => showToast('Voice capture failed — type instead', 'warn');
            try {
                rec.start();
                showToast('Listening… speak now', 'success');
            } catch (err) {
                showToast('Voice input unavailable', 'warn');
            }
        }

        function toggleModalStdWorkSection() {
            const body = document.getElementById('modal-stdwork-body');
            const icon = document.getElementById('modal-stdwork-chevron');
            if (!body) return;
        ...
        function renderModalStdWorkFields() {
            const section = document.getElementById('modal-stdwork-section');
            const lap = modalLapIndex != null ? laps[modalLapIndex] : null;
            if (!section) return;
            const show = lap && isRegularElement(lap);
            section.classList.toggle('hidden', !show);
            if (!show) return;

            const readOnly = isReadOnlyMode();
            const vaCat = normalizeVaCategory(lap.vaCategory);
            const vaSel = document.getElementById('modal-va-category');
            if (vaSel) {
                vaSel.value = vaCat;
                vaSel.disabled = readOnly;
            }
            const kp = document.getElementById('modal-key-points');
            const rw = document.getElementById('modal-reasons-why');
            if (kp) {
                kp.value = lap.keyPoints || '';
                kp.disabled = readOnly;
                kp.readOnly = readOnly;
            }
            if (rw) {
                rw.value = lap.reasonsWhy || '';
                rw.disabled = readOnly;
                rw.readOnly = readOnly;
            }
            document.querySelectorAll('.modal-voice-btn').forEach(btn => {
                btn.classList.toggle('hidden', readOnly);
            });
        }

        function syncModalReadOnlyState() {
            const readOnly = isReadOnlyMode();
            const timerBlock = document.getElementById('modal-timer-controls');
            const howTo = document.getElementById('modal-how-to-use');
            if (timerBlock) timerBlock.classList.toggle('hidden', readOnly);
            if (howTo) howTo.classList.toggle('hidden', readOnly);
            const subtitleEl = document.getElementById('modal-element-subtitle');
            if (subtitleEl && modalLapIndex != null) {
                subtitleEl.textContent = readOnly ? 'STANDARD WORK — VIEW ONLY' : 'ELEMENTAL TIME STUDY';
            }
            renderModalStdWorkFields();
        }

        function onModalStdWorkFieldChange(field, value) {
            if (isReadOnlyMode() || modalLapIndex == null || !laps[modalLapIndex]) return;
            if (field === 'vaCategory') {
                laps[modalLapIndex].vaCategory = normalizeVaCategory(value);
            } else if (field === 'keyPoints' || field === 'reasonsWhy') {
                laps[modalLapIndex][field] = value;
            }
            saveToStorage();
        }
        ...
        function openElementalModal(index) {
            ...
            if (subtitleEl) subtitleEl.textContent = isReadOnlyMode() ? 'STANDARD WORK — VIEW ONLY' : 'ELEMENTAL TIME STUDY';

            resetModalTimerToZero();
            renderModalObservations();
            syncModalReadOnlyState();
        ...
        function renderModalObservations() {
            ...
            target.observations.forEach((obs, idx) => {
                ...
                const deleteBtn = readOnly ? '' :
                    `<button onclick="deleteModalObs(${idx}); event.stopImmediatePropagation()" class="text-red-400 hover:text-red-500 px-2 min-h-[36px] min-w-[36px]">×</button>`;
                row.innerHTML = `
        ...
        function copyElementsFromStudy(studyId) {
            ...
            laps = source.laps.map((lap, i) => normalizeLapFields({
                number: i + 1,
                name: lap.name || `Element ${i + 1}`,
                split: 0,
                observations: [],
                workType: lap.workType || 'regular',
                keyPoints: lap.keyPoints || '',
                reasonsWhy: lap.reasonsWhy || '',
                vaCategory: lap.vaCategory || 'value-added'
            }));
            cycleTimes = [];
            elapsedTime = 0;
            isRunning = false;
        ...
        function loadStudyIntoCapture(id, canEditParam) {
            ...
            laps = study.laps
                ? JSON.parse(JSON.stringify(study.laps)).map(normalizeLapFields)
                : [];
        ...
        function exportKaizenReport() {
            ...
            const vaBreakdown = computeStudyVaBreakdown(laps, allowance);
            const pctVaLabel = vaBreakdown.pctVa != null ? `${vaBreakdown.pctVa}%` : '';
            let csv = 'Element#,Element Name,Work Type,VA Category,Key Points,Reasons Why,Observed Low (ms),Observed Avg (ms),Observed High (ms),Standard Time (ms),Std Dev (ms),FLUCT %,Obs Count,Study %VA,Notes\n';
            laps.forEach((lap, idx) => {
                ...
                csv += [
                    ...
                    isRegularElement(lap) ? getVaCategoryFullLabel(lap.vaCategory) : 'N/A',
                    `"${(lap.keyPoints || '').replace(/"/g, '""')}"`,
                    `"${(lap.reasonsWhy || '').replace(/"/g, '""')}"`,
                    st.min || 0,
                    st.avg || 0,
                    st.max || 0,
                    stdTime,
        ...
        function exportStudyCSV(studyId) {
            ...
            const wiCol = study.workInstructions?.length
                ...
                : 'N/A';
            const studyVa = computeStudyVaBreakdown(study, allowance);
            const pctVaCol = studyVa.pctVa != null ? `${studyVa.pctVa}%` : '';
            let csv = 'StudyID,StudyName,Area,Station,Shift,WorkInstructions,Date,CreatedBy,TotalTime_ms,AllowancePct,Mode,Item#,ItemName,WorkType,Time_ms,Time_formatted,ObsCount,Min_ms,Max_ms,Avg_ms,Std_ms,Standard_ms,Standard_formatted,VA_Category,KeyPoints,ReasonsWhy,Study_PctVA\n';
            ...
            if (study.workflowMode === 'cycles' && study.cycleTimes && study.cycleTimes.length) {
                ...
                study.cycleTimes.forEach((t, i) => {
                    csv += [sid, name, study.area, stationCol, shiftCol, wiCol, study.date||'', study.createdBy, study.totalTime, allowance, 'CYCLES', i+1, `Cycle ${i+1}`, 'regular', t, formatTime(t), 1, t, t, t, 0, stdCycle, formatTime(stdCycle), '', '', '', pctVaCol].join(',') + '\n';
                });
                if (study.laps) {
                    study.laps.forEach((lap, i) => {
                        ...
                        csv += [sid, name, study.area, stationCol, shiftCol, wiCol, study.date||'', study.createdBy, study.totalTime, allowance, 'CYCLES_ELEMENT',
                            ...
                            st.count, st.min||0, st.max||0, st.avg||0, st.std||0, stdT, formatTime(stdT),
                            isRegularElement(lap) ? getVaCategoryFullLabel(lap.vaCategory) : 'N/A',
                            `"${(lap.keyPoints||'').replace(/"/g,'""')}"`,
                            `"${(lap.reasonsWhy||'').replace(/"/g,'""')}"`,
                            pctVaCol].join(',') + '\n';
        ...
        function getCaptureStudySnapshot() {
            ...
            return {
                ...
                shiftCustom,
                date: document.getElementById('study-date')?.value || new Date().toISOString().split('T')[0],
                lastModified: captureStudyId
                    ? (studies.find(s => s.id === captureStudyId)?.lastModified || new Date().toISOString())
        ...
        function buildStdDocElementsTable(study, allowance) {
            ...
            return `
                ...
                ${buildStdDocVaSummaryHtml(study, allowance)}
                ...
                <table class="std-doc-table std-doc-table-bordered">
                    <thead><tr>
                        <th>#</th><th>Work Element</th><th>Low</th><th>Avg</th><th>High</th><th>Std Dev</th><th>Std Time</th><th>FLUCT %</th><th>VA</th><th>Remarks</th>
                    </tr></thead>
                    <tbody>${rows}
                        <tr class="std-doc-total">
                            ...
                            <td colspan="3">Compare to WACT from cycles</td>
                        </tr>
                    </tbody>
                </table>`;
        }
        ...
        function buildStdDocVaSummaryHtml(study, allowance) {
            const breakdown = computeStudyVaBreakdown(study, allowance);
            if (!breakdown.totalMs) return '';
            const vaTime = breakdown.vaMs > 0 ? formatTime(breakdown.vaMs) : '—';
            const nnvaTime = breakdown.nnvaMs > 0 ? formatTime(breakdown.nnvaMs) : '—';
            const wasteTime = breakdown.wasteMs > 0 ? formatTime(breakdown.wasteMs) : '—';
            const pctLabel = breakdown.pctVa != null ? `${breakdown.pctVa}%` : '—';
            return `
                <div class="std-doc-va-summary">
                    <span class="std-doc-va-pill va"><strong>%VA of WACT:</strong> ${pctLabel}</span>
                    <span class="std-doc-va-pill"><strong>VA:</strong> ${vaTime}</span>
                    <span class="std-doc-va-pill nnva"><strong>NNVA:</strong> ${nnvaTime}</span>
                    <span class="std-doc-va-pill waste"><strong>Waste:</strong> ${wasteTime}</span>
                </div>`;
        }
        ...
        function buildStdDocElementRow(lap, idx, allowance, includeInWact) {
            ...
            const typeTag = lap.workType && lap.workType !== 'regular'
                ...
                : '';
            const vaTag = isRegularElement(lap)
                ? `<span class="std-doc-va-tag ${normalizeVaCategory(lap.vaCategory).replace(/-/g, '_')}">${getVaCategoryLabel(lap.vaCategory)}</span> `
                : '';
            const keyPoints = lap.keyPoints?.trim() ? escapeStdDocHtml(lap.keyPoints.trim()) : '';
            const reasonsWhy = lap.reasonsWhy?.trim() ? escapeStdDocHtml(lap.reasonsWhy.trim()) : '';
            const detailRows = [
                keyPoints ? `<tr class="std-doc-kp-row"><td></td><td colspan="9"><span class="std-doc-kp-lbl">Key Points</span> ${keyPoints}</td></tr>` : '',
                reasonsWhy ? `<tr class="std-doc-kp-row"><td></td><td colspan="9"><span class="std-doc-kp-lbl">Reasons Why</span> ${reasonsWhy}</td></tr>` : ''
            ].join('');
            return `<tr>
                <td class="std-doc-num">${num}</td>
                <td>${typeTag}${vaTag}${name}</td>
                ...
                <td class="mono std-doc-va-col">${isRegularElement(lap) ? getVaCategoryLabel(lap.vaCategory) : '—'}</td>
                <td class="std-doc-remarks">${keyPoints || reasonsWhy ? 'See below' : ''}</td>
            </tr>${detailRows}`;
        }
        ...
        function buildStandardizationDocumentHTML(study) {
            ...
            const lastUpdated = study.lastModified
                ? new Date(study.lastModified).toLocaleString()
                : (study.date || generatedAt);
            ...
            if (study.workflowMode === 'cycles' && study.cycleTimes?.length) {
                ...
                elementsSection = `
                    ...
                    <table class="std-doc-table std-doc-table-bordered">
                        ...
                        <tbody>${rows || `<tr><td colspan="10" style="color:#71717a;">No regular elements recorded.</td></tr>`}
                            <tr class="std-doc-total">
                                <td colspan="2">Total Standard Time (WACT)</td>
                                <td colspan="4"></td>
                                <td class="mono std-doc-std">${totalStdMs > 0 ? formatTime(totalStdMs) : '—'}</td>
                                <td colspan="3">sec/unit pace target</td>
                            </tr>
                        </tbody>
                    </table>`;
            } else {
            ...
            return `<div class="std-doc-paper">
                ...
                <div class="std-doc-footer">Generated by StdWork v${STDWORK_VERSION} · ${escapeStdDocHtml(generatedAt)}<br><span class="std-doc-footer-meta">Last updated · ${escapeStdDocHtml(lastUpdated)}</span></div>
            </div>`;
        }
        ...
        function buildStdDocExportNode(paper) {
            ...
            exportNode.style.padding = '0.6in 0.65in';
            exportNode.style.fontSize = '11pt';
            exportNode.style.transform = 'none';
            exportNode.style.boxShadow = 'none';
            exportNode.style.margin = '0';
            exportNode.querySelectorAll('.std-doc-table, .std-doc-header-grid, .std-doc-kpi-grid').forEach(el => {
        ...
        async function downloadStandardizationDocumentPdf() {
            ...
            const btns = [...document.querySelectorAll('.std-doc-download-action')];
            const origHtmls = btns.map(btn => btn.innerHTML);
            btns.forEach(btn => {
                btn.disabled = true;
            ...
            try {
                ...
                await html2pdf().set({
                    ...
                    filename,
                    pagebreak: { mode: ['css', 'legacy'], avoid: ['.std-doc-wi-step', '.std-doc-table', 'tr', '.std-doc-header-grid'] },
                    image: { type: 'jpeg', quality: 0.97 },
                    html2canvas: { scale: 2, useCORS: true, logging: false, backgroundColor: '#ffffff', windowWidth: 816 },
                    jsPDF: { unit: 'in', format: 'letter', orientation: 'portrait' }
                }).from(built.exportNode).save();

                showToast('PDF saved — check Downloads or Files');
            } catch (err) {
                console.warn('StdWork PDF download fallback:', err);
                if (typeof html2pdf === 'undefined') {
                    showToast('PDF library unavailable — opening print dialog', 'info');
                }
                ...
                btns.forEach((btn, i) => {
                    ...
                    if (origHtmls[i]) btn.innerHTML = origHtmls[i];
                });
            }
        }